...
local function parse_accept_header(headers)
  ...
  return accept_header, content_type, template
end

return function(ctx)
  kong.log.debug("Handling error")

  local headers = kong.request.get_headers()
  local accept_header, content_type, template = parse_accept_header(headers)

  local status = status_code()
  local message = ctx and ctx.error_body and ctx.error_body.message or BODIES["s" .. status] or fmt(HEADERS.default, status)

  if find(accept_header, TYPE_GRPC, nil, true) == 1 then
    kong.log.debug("GRPC error - no need to do anything")
    return kong.response.exit(status, { message = message }, headers)
  end
  ...
  if not ngx.headers_sent then
    -- Do not use this (last checked in v2.4.1) as this won't correctly parse
    -- complex headers presented by browsers like:
    -- `text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.9`
    -- Our simplistic version has been working thus far so we'll stick with it
    -- until you read this and start swearing at me
    -- local mime_type = utils.get_mime_type(accept_header)
    ngx.header["Content-Type"] = content_type

    if type(ctx.error_headers) == 'table' then
  ...
  -- Return error based on `accept_header`
  if find(accept_header, TYPE_HTML, nil, true) then
    kong.log.debug("Returning HTML error template")

    local title = status..": "..(HEADERS["s" .. status] or fmt(HEADERS.default, status))
    local brand = ctx and ctx.brand or "statpro"

    local custom_html_template = require "resty.template"
    custom_html_template.render(brand.."/error.html", { title = title, brand = brand, user_details = ctx and ctx.user_details, message = message, request_id = request_id, status = status })
  else
    kong.log.debug("Returning non-HTML error template - "..(template or "nil"))

    ngx.say(fmt(template, request_id, message))
  end
end
